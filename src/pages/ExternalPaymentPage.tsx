...
import { isLinkExpired } from "@/lib/externalLinkGenerator";
...
const ExternalPaymentPage = () => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold mb-2">انتهت صلاحية الرابط</h2>
          <p className="text-muted-foreground mb-4">هذا الرابط لم يعد متاحاً</p>
          <Button onClick={() => navigate('/')}>
            العودة للصفحة الرئيسية
          </Button>
        </div>
  ...
  // SEO metadata
  const seoTitle = link.metadata?.title || (isShipping 
    ? `تتبع وتأكيد الدفع - ${serviceName}` 
    : `حجز شاليه - ${payload.chalet_name}`);
  const seoDescription = link.metadata?.description || (isShipping
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="text-center mb-8">
              <Badge className="text-lg px-6 py-2 bg-gradient-primary">
                ...
                <span>رابط خارجي موثّق ومحمي</span>
              </Badge>
            </div>
            ...
            <Card className="overflow-hidden shadow-elevated">
              ...
              <div
                ...
                style={{
                  background: countryData ? `linear-gradient(135deg, ${countryData.primaryColor}, ${countryData.secondaryColor})` : 'linear-gradient(135deg, #3b82f6, #1d4ed8)',
                }}
              >
                <div className="absolute inset-0 bg-black/20" />
                <div className="absolute bottom-4 right-6 text-white">
                  <h1 className="text-3xl font-bold">{displayName}</h1>
                  <p className="text-lg opacity-90">{countryData?.nameAr || link.countryCode}</p>
                </div>
              </div>
              ...
              <div className="p-8">
                    ...
                    <>
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-muted-foreground text-sm">
                            {formatCurrency(payload.cod_amount, countryData?.currency || 'AED')}
                          </p>
                        </div>
                      </div>
                    </>
                    ...
                    <>
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-muted-foreground text-sm">
                            {formatCurrency(payload.price_per_night, countryData?.currency || 'AED')}
                          </p>
                        </div>
                      </div>
                    </>
                ...
                <div className="bg-gradient-primary p-6 rounded-xl text-primary-foreground mb-6">
                  ...
                  <p className="text-5xl font-bold mb-2">
                    {formatCurrency(
                      isShipping ? payload.cod_amount : payload.total_amount, 
                      countryData?.currency || 'AED'
                    )}
                  </p>
                ...
                <Button
                  ...
                  onClick={() => navigate(`/external/pay/${link.externalId}`)}
                >
